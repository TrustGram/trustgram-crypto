...
import { generateKeyPair, exportPublicKey, importPublicKey, deriveBits, hkdfExpand, hkdf, aesEncrypt, aesDecrypt } from "./primitives.ts";
const MAX_SKIP = 100; // max skipped messages to store
// -------------------------
// Initialize ratchet
// -------------------------
// Called by the session initiator (Alice) after X3DH
export async function initSenderRatchet(masterSecret, theirPublicKeyB64) {
    ...
    const { key1: rootKey, key2: sendChainKey } = await hkdfExpand(masterSecret, dh, "TrustGram_Ratchet_v1");
    return {
        dhSendKey,
        dhRecvKey: theirPub,
        rootKey,
...
export async function initReceiverRatchet(masterSecret) {
    const dhSendKey = await generateKeyPair();
    return {
        dhSendKey,
        dhRecvKey: null,
...
// Decrypt
// -------------------------
export async function ratchetDecrypt(state, message) {
    // Check if we have a skipped key for this message
    const skipped = state.skippedKeys.find(k => k.dhKey === message.dhPub && k.n === message.n);
    if (skipped) {
...
async function dhRatchetStep(state, theirPub) {
    ...
    const { key1: rootKey1, key2: recvChainKey } = await hkdfExpand(state.rootKey, dh1, "TrustGram_Ratchet_v1");
    ...
    const { key1: rootKey2, key2: sendChainKey } = await hkdfExpand(rootKey1, dh2, "TrustGram_Ratchet_v1");
    return {
        ...state,
