...
import { generateKeyPair, exportPublicKey, importPublicKey, deriveBits, hkdf } from "./primitives.ts";
...
export async function exportPublicBundle(bundle) {
    return {
        ...
        oneTimePreKeys: await Promise.all(bundle.oneTimePreKeys.map(kp => exportPublicKey(kp.publicKey)))
    };
}
// -------------------------
// X3DH sender side (Alice)
// -------------------------
export async function x3dhSend(myIdentityKey, recipientBundle) {
...
// -------------------------
export async function x3dhReceive(myIdentityKey, mySignedPreKey, myOneTimePreKey, senderIdentityKeyB64, senderEphemeralKeyB64) {
